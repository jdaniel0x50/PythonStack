...
def validate_date(validation_errors):
    if not session['birthdate'] < session['date_max']:
        ...
        validation_errors = flash_message(flash_msg)
    hund20_years = datetime.date.today() - datetime.timedelta(days=120*365.24)
    hund20_years = hund20_years.isoformat()
    if not session['birthdate'] > hund20_years:
        flash_msg = construct_flash_message("past_date", "birthdate")
        validation_errors = flash_message(flash_msg)
    return validation_errors
